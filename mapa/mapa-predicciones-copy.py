...
from config.datos import cargar_datos, filtrar_geojson, cargar_geojson, calcular_porcentaje_robos, \
    contar_robos_por_distrito, guardar_porcentaje_robos_csv, cargar_datos_async, cargar_geojson_async
...
async def main():
    ...
    mapa: Map = crear_mapa()
    geojson_data = await cargar_geojson_async(ruta_geojson)
    datos_incidentes = await cargar_datos_async("http://localhost:3000/robbery")
    ...
    fin = time.time()
    print(f"El proceso tardo: {fin - inicio} segundos")
